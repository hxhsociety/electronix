...
struct PcsimParams {
    ...
    pad_d_mm:    f64,
    solder_h_mm: f64,
    /// Solder alloy name: "SAC305" | "SAC405" | "SnPb37"
    solder_alloy: Option<String>,
    /// Per-material property overrides.  Key = material_id as string;
    /// value = flat map of property_name → f64 (e.g. "ANAND_S0_MPA" → 1.95).
    material_overrides: Option<std::collections::HashMap<String, std::collections::HashMap<String, f64>>>,
}
...
async fn write_pcsim_file(params: PcsimParams) -> Result<String, String> {
    ...
    let text = build_pcsim_text(
        ...
        params.solder_alloy.as_deref().unwrap_or("SAC305"),
        params.material_overrides.as_ref().unwrap_or(&empty_overrides),
    );
...
fn build_pcsim_text(
    ...
    solder_h_mm:        f64,
    solder_alloy:       &str,
    material_overrides: &std::collections::HashMap<String, std::collections::HashMap<String, f64>>,
) -> String {
    ...
    s.push_str(&format!("*SOLDER_ALLOY {solder_alloy}\n\n"));
    // Emit per-material overrides (only if any were provided)
    for (mat_id, props) in material_overrides {
        if props.is_empty() { continue; }
        s.push_str(&format!("*MATERIAL_OVERRIDE, ID={mat_id}\n"));
        let mut keys: Vec<_> = props.keys().collect();
        keys.sort();   // deterministic file output
        for k in keys {
            s.push_str(&format!("{}={}\n", k, props[k]));
        }
        s.push('\n');
...
async fn run_solver_auto(
    ...
    ambientC:   f64,
    curveName:  String,
    profilePts: Vec<(f64, f64)>,   // [(time_min, temp_c), …]
    nCycles:    usize,
    ...
    // ── Step 2: write pcsim using the sim-case parameters ────────────────────
    let sim_dir = appdata_root()
    ...
    // Use caller-supplied profile; fall back to JEDEC-B if none provided
    let pts_to_use: Vec<(f64, f64)> = if profilePts.is_empty() {
        vec![(0.0, 25.0), (6.0, -55.0), (21.0, -55.0),
             (34.0, 125.0), (49.0, 125.0), (56.0, 25.0)]
    } else {
        profilePts
    };
    let cycles_to_use = if nCycles == 0 { 5 } else { nCycles };
    let pcsim_text = build_pcsim_text(
        &pcprep_rel, ambientC, &curveName, &pts_to_use, cycles_to_use, 0.5, 0.30,
        "SAC305", &std::collections::HashMap::new(),
    );
...
enum PcresRecord {
    // rename_all on the container renames variant tags ("Creep" → "creep") but NOT
    // the fields inside struct variants.  Each variant needs its own rename_all to
    // get camelCase field names in the JSON payload the frontend receives.
    #[serde(rename_all = "camelCase")]
    Creep {
...
fn read_pcres_data(path: &str) -> Result<Vec<PcresRecord>, String> {
    ...
    for _ in 0..n {
        ...
        for &ft in &field_types {
            match ft {
                // Sanitise floats: NaN / Inf cannot be serialised by serde_json and
                // would cause the entire IPC call to fail.  Zero is the safe fallback.
                FTYPE_F32 => { let v = read_f32!(); fvals.push(if v.is_finite() { v } else { 0.0 }); }
...
/// Returns `Some(version_string)` when a newer release is available on GitHub,
/// `None` when the app is already up-to-date, or an error string on failure.
#[tauri::command]
